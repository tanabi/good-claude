...
Paste is the point. good-claude exists to be pasted into and copied out of, and everything on the input path is built for that.

Text you paste is read as raw bytes and assembled into lines by the client itself, with no line editor in the way. That is not an aesthetic choice. Python's line editor is libedit on macOS and on any python3 installed by Homebrew or linuxbrew, and on macOS libedit rewrites a large paste into repeated fragments of itself -- the same 2207-character document came back at exactly the right length with the wrong contents in it. GNU readline corrupted the same paste too, in a different way. A plain read of the same text on the same terminal, over the same ssh connection, was byte-for-byte correct. So the line editor is gone.

The terminal is also taken out of canonical mode, where the kernel assembles lines for you. That path caps a single line at `MAX_CANON` -- 1024 bytes on macOS -- and silently discards whatever does not fit, which would quietly eat the tail of any pasted paragraph that has no line breaks in it.

What you get: pasted text arrives byte for byte, at any size, whether or not a turn is running when it lands. Tabs stay tabs, so indented code pastes as indented code. UTF-8 survives. Backspace, Ctrl-U, Ctrl-W and Ctrl-D all work, provided by the client rather than the terminal. Ctrl-C still signals normally.

What you give up: command history, and arrow-key editing within a line. Pressing an arrow does nothing at all -- the sequence is recognised and dropped rather than landing in your message as junk. Since Enter starts a new line here and never sends, you were never able to go back and edit an earlier line anyway.
...
The terminal is held in raw mode while the client runs and put back when it exits, by both a `finally` and an `atexit` handler. A hard kill -- `kill -9`, or a crash that runs neither -- leaves your terminal without echo. Typing `stty sane` blind fixes it.

Terminal paste behavior needs a real terminal to exercise. It is covered by a pty harness that checks a multi-kilobyte paste against its md5, plus long lines, editing keys, tabs and UTF-8. What that harness cannot cover is the full range of real terminals, multiplexers and ssh setups. Notably, the corruption that motivated all of this does not reproduce on Linux at all, on either line-editor backend, at any speed.
